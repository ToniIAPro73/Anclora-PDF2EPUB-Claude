...
import { createClient, SupabaseClient } from "npm:@supabase/supabase-js@2.36.0";
...
const getEnv = (key: string): string | null => Deno.env.get(key) ?? null;
...
const isValidEmail = (email: string) => /@/.test(email) && email.length > 5;
...
Deno.serve(async (req: Request) => {
  try {
    ...
    const createdUser: any = authData; // contiene id y detalles del usuario
